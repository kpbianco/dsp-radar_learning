...
Project 73 combines two explicit TX positions with four RX positions into
...
Project 76 transmits an explicit LFM pulse at every aperture position, inserts
three delayed point-target echoes with preserved two-way carrier phase, and
...
Project 78 holds one stationary target under a long squinted aperture, measures
its exact curved slant-range ridge in metres, stored bins, and physical
resolution cells, then explicitly linearly interpolates each aperture row to a
common range. Fixed-bin and path-following processors use identical complex
...
Project 80 converts millimetre-scale line-of-sight platform error into a common
two-way aperture phase screen, measures coherent-peak and entropy loss across
wavelength-fraction and error-correlation sweeps, and applies an explicit
